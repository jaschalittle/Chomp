...
///          protocol over UART/RS232/RS485.  Supports 16-channel and single 
///          channel sensors.
///
...
public:
	Leddar16() 
	: BaudRate(DEFAULT_BAUD_RATE), SlaveAddress(DEFAULT_ADDRESS), NbDet(0), TxEn_Pin(0), TxEn_Action(0) 
	{ }
	
	Leddar16( unsigned long Baud, unsigned char Addr = DEFAULT_ADDRESS, unsigned char Pin = 0, unsigned char Action = 0) 
	: BaudRate(Baud), SlaveAddress(Addr), NbDet(0), TxEn_Pin(Pin), TxEn_Action(Action)  
	{ }
...
public:
	...
	LeddarOne( unsigned long Baud, unsigned char Addr = DEFAULT_ADDRESS, unsigned char Pin = 0, unsigned char Action = 0) 
	: BaudRate(Baud), SlaveAddress(Addr), NbDet(0), TxEn_Pin(Pin), TxEn_Action(Action)  
	{ }
