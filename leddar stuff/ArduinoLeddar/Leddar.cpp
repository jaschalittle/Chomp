...
///          via Modbus RTU protocol over UART/RS232/RS485.	
///
...
char Leddar16::getDetections() 
{
	...
	if (len < 256 && receivedData[1] == 0x41 && receivedData[0] == SlaveAddress)
	{
		...
		TimeStamp = ((unsigned long)receivedData[len-5])<<24 + ((unsigned long)receivedData[len-6])<<16 + 
		            ((unsigned long)receivedData[len-5])<<8 + receivedData[len-5];
...
char LeddarOne::getDetections() 
{
	...
	while (millis()-startTime < 10) 
	{
		if (Serial.available())
		{
			if (len >= 19) return ERR_LEDDAR_BAD_RESPONSE; 
			dataBuffer[len++] = Serial.read();
			startTime = millis();
		}
	}

	if (len == 19 && dataBuffer[1] == 0x04 && dataBuffer[0] == SlaveAddress)
	{
		// Check CRC
